...
// NewS3 wraps an S3 client and records all requests and responses.
func NewS3(cli s3iface.S3API, recorder *spectest.Recorder) s3iface.S3API {
	return &s3Recorder{
		S3API:    cli,
		recorder: recorder,
	}
...
func (r s3Recorder) GetObject(input *s3.GetObjectInput) (*s3.GetObjectOutput, error) {
	...
	if output != nil {
		if output.Body != nil {
			...
			output.Body = io.NopCloser(bytes.NewBuffer(data))
...
func (r s3Recorder) recordInput(operation, body string) {
	r.recorder.AddMessageRequest(spectest.MessageRequest{
...
func (r s3Recorder) recordOutput(operation, body string, err error) {
	if err != nil {
		r.recorder.AddMessageResponse(spectest.MessageResponse{
			Source:    "S3",
